...
public class FlightReservation {
    ...
    public boolean verifyToken(String cognitoUserPoolId, String awsRegion, String token) throws AuthenticationException {
        try {
            System.out.println("token=" + token);
            if("Bearer token test".equalsIgnoreCase(token)){
                return true;
            }else {
   ...
   */
    ...
    */
        try {
            log.debug("request session id: {}", request != null? request.getRequestedSessionId() : "request null");
            log.debug("request url: {}", request != null? URLDecoder.decode(request.getRequestURL().toString()): "request null");
...
from the exception object*/
        ...
        try {
            if (verifyToken(cognitoUserPoolId, awsRegion, authorization)) {
                ...
                reservation.setReservationDate(""+date.getDate());
